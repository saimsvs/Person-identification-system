...
for subfoldernum in range_subfol:
    ...
    output_folder = f"person{subfoldername}_lbp_concat"
    ...
    for imagenum in images:
            ...
            #lbp implementations for more accuracy
            lbpeyeimage = local_binary_pattern(edetectedr, 8, 1, method='uniform')
            lbpfaceimage = local_binary_pattern(fdetectedr, 24, 3, method='uniform')
            lbpnoseimage = local_binary_pattern(ndetectedr, 16, 2, method='uniform')
            lbpmouthimage = local_binary_pattern(mdetectedr, 16, 2, method='uniform')


            #vectors
            eye_vector = lbpeyeimage.ravel()
            face_vector = lbpfaceimage.ravel()
            nose_vector = lbpnoseimage.ravel()
            mouth_vector = lbpmouthimage.ravel()
            ...
            output_csv_path = os.path.join(output_folder, f"{os.path.splitext(imagenum)[0]}_lbp_concat.csv")
            np.savetxt(output_csv_path, concatenated_2d, delimiter=',', fmt='%d')
            print(f"Saved concatenated LBP vector as CSV to {output_csv_path}")
