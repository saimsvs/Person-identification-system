...
# Helper: dim the image (factor<1 dims the image)
def dim_image(img, factor=0.5):
...
for person_id in range(1, 12):
    ...
    for img_name in images:
        ...
        # Dim image
        dimmed_img = dim_image(img, factor=0.5)

        # Save original  and all augmentations
        ...
        cv2.imwrite(os.path.join(output_person_folder, f"{base_name}_dim.jpg"), dimmed_img)
